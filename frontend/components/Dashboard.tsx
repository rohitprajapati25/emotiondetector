...
export default function DashboardContent() {
    ...
    /* =========================================================
       🔥 VISITOR TRACKING (ADDED – NEXT.JS SAFE)
    ========================================================= */

    useEffect(() => {
        if (!sessionId) return;

        // Register visitor (increment total + live)
        fetch(`${API_BASE_URL}/status?session_id=${sessionId}`, {
            cache: "no-store"
        }).catch(() => { });
    ...
    useEffect(() => {
        if (!isLocalHost && isRunning && navigator.mediaDevices) {
    ...
    }, [isRunning, isMobile, isLocalHost]);
        ...
        const fetchStatus = async () => {
            try {
                const res = await fetch(
                    `${API_BASE_URL}/status?session_id=${sessionId}`,
                    { cache: "no-store" }
                );
                const json = await res.json();
                if (alive) {
                ...
                if (alive) setError("Cloud Brain: Connecting...");
            }
        };
    ...
    return (
        <main className="min-h-screen p-6 bg-[#020617] text-white">
            ...
            {error && (
                <div className="fixed bottom-10 left-1/2 -translate-x-1/2 glass-dark px-8 py-4 rounded-2xl border border-red-500/40 flex items-center gap-4 shadow-2xl">
                    <AlertCircle className="w-5 h-5 text-red-500" />
